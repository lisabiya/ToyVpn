...
package xyz.hexene.localvpn.toy;
...
public class ToyVpnClient extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        String sts = TextUtils.join(", ", prefs.getStringSet(Prefs.PACKAGES, Collections.emptySet()));
        packages.setText(sts);
        ...
        findViewById(R.id.connect).setOnClickListener(v -> {
            ...
            final Set<String> packageSet = Stream.of(packages.getText().toString().split(","))
                    .map(trim -> trim)
                    .filter(s -> !s.isEmpty())
    ...
    private boolean checkPackages(Set<String> packageNames) {
        final boolean hasCorrectPackageNames = packageNames.isEmpty() ||
                Stream.of(getPackageManager().getInstalledPackages(0))
